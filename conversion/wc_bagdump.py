...
from wc_bagutils import *

#  JKM
import code
JKM = True # debug switch
...
#JKM
def chair2dict(msg, chair_dict):
    chair_dict["timestamp"].append(msg.header.stamp.to_nsec())
    chair_dict["chairRx"].append(msg.data.data)
...
def main():
    ...
    if JKM: print("\n JKM Filter Topics:" + str(filter_topics))
    if JKM: print("\n JKM Input Dir of Bag Files: " + indir)

    bagsets = find_bagsets(indir, filter_topics=filter_topics)

    if JKM: print("\n JKM Bagsets: " + str(bagsets))
    ...
    for bs in bagsets:
        ...
        if include_others:
            ...
            # JKM
            chair_cols = ["timestamp", "chairRx"]
            chair_dict = defaultdict(list)
            ...
            if topic in CAMERA_TOPICS:
                ...
                if include_others:
                    if topic == GEAR_TOPIC:
                        ...
                        stats['msg_count'] += 1
                    # JKM
                    elif topic == JKM_CHAIR_TOPIC:
                        chair2dict(msg, chair_dict)
                        stats['msg_count'] += 1
        ...
        if include_others:
            ...
            # JKM
            chair_csv_path = os.path.join(dataset_outdir, 'chair.csv')
            chair_df = pd.DataFrame(data=chair_dict, columns=chair_cols)
            chair_df.to_csv(chair_csv_path, index=False)

        # JKM - add an interrupt here
        ...
        if include_images and gen_interpolated:
            # TODO JKM - make change here later - ... in progress
            if JKM: print( " JKM: Starting generate interpolated ")
            ...
            if JKM: print(" camera_df after pd.datetime")
            if JKM: print("JKM camera_df ", camera_df) 
            #
            # JKM - round to the millisec                     
            camera_df['timestamp'] = camera_df['timestamp'].dt.round('10ms') 
            if JKM: print("JKM camera_df.round", camera_df['timestamp'] )
            #
            camera_df.set_index(['timestamp'], inplace=True)  # set the timestamps to become the index into table
            if JKM: print("JKM camera_df.set_index done", camera_df)
            camera_df.index.rename('index', inplace=True) # coverts type and class of the index, mutates in place
            if JKM: print("JKM camera_df.index.rename done", camera_df) 
 
            # JKM 
            chair_df['timestamp'] = pd.to_datetime(chair_df['timestamp'])
            chair_df['timestamp'] = chair_df['timestamp'].dt.round('10ms')
            chair_df.set_index(['timestamp'], inplace=True)
            chair_df.index.rename('index', inplace=True)
            #           
            # Below Comment out JKM 
            ...
            merged = functools.reduce(lambda left, right: pd.merge(
                left, right, how='outer', left_index=True, right_index=True), [camera_df, chair_df])
            if JKM: print("JKM merged ", merged)
            merged.interpolate(method='time', inplace=True)
            if JKM: print("JKM merged interpolated ", merged)
            ...
            if JKM: print("JKM filtered ", filtered)
            filtered.fillna(0.0, inplace=True)
            #if JKM: print("JKM filtered with fillna 0.0", filtered)
            ...
            if JKM: print("JKM filtered with filtered_cols", filtered)
